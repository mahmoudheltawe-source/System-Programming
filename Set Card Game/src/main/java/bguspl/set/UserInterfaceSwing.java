...
public class UserInterfaceSwing extends JFrame implements UserInterface {
    ...
    public UserInterfaceSwing(Logger logger, Config config, Player[] players) {
        ...
        InputManager inputManager = new InputManager(logger, config, players);
        addKeyListener(inputManager);
        registerKeyBindings(config, inputManager);
    ...
    private void registerKeyBindings(Config config, InputManager inputManager) {
        InputMap inputMap = getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
        ActionMap actionMap = getRootPane().getActionMap();
        for (int player = 0; player < config.players; ++player) {
            for (int keyCode : config.playerKeys(player)) {
                if (keyCode <= 0) {
                    continue;
                }
                String actionName = "player-key-" + keyCode;
